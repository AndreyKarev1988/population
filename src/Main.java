...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        long adultAge = persons.stream()
                .filter(person -> person.getAge() < 18).count();


        List<String> recruitsForArmy = persons.stream().filter(person -> (person.getAge() >= 18 && person.getAge() <= 27))
                .filter(person -> person.getSex() == Sex.MAN).map(Person::getFamily)
                .collect(Collectors.toList());


        Collection<Person> ableForWork = persons.stream()
